...
// [[Rcpp::export]]
Rcpp::DataFrame rcpp_jaccard_single_Int64(
  ...
  bool debug=false) {
  //get sizes
  int n = kmer_counts_q_Int64.size();
  int m = kmer_counts_t_Int64.size();
  if (debug) {
  ...
  //kmers
  std::vector<std::vector<std::int64_t>> Qkmers_sorted(n);
  std::vector<std::vector<std::int64_t>> Tkmers_sorted(m);
  //kmer counts
  ...
  Qkmers_sorted = convertInnerNamesRcppListOfIntegerVectorToStdVectorInt64(kmer_counts_q_Int64);
  Tkmers_sorted = convertInnerNamesRcppListOfIntegerVectorToStdVectorInt64(kmer_counts_t_Int64);
  Qkmers_counts_sorted = convertRcppListOfIntegerVectorToStdVector(kmer_counts_q_Int64);
  Tkmers_counts_sorted = convertRcppListOfIntegerVectorToStdVector(kmer_counts_t_Int64);
  //create kmerMaps
  std::map<std::int64_t, std::vector<int>> QkmerMap;
  std::map<std::int64_t, std::vector<int>> TkmerMap;
  //create vector to keep separate maps
  auto start_QkmerMap = std::chrono::steady_clock::now();
  std::vector<std::map<std::int64_t, int>> QkmerMap_n(n);
  QkmerMap_n.resize(n);
  for (int i=0; i < n; ++i) {
    std::map<std::int64_t, int> QkmerMap_i;
    std::vector<std::int64_t> Qkmers_i_sorted = Qkmers_sorted[i];
    QkmerMap_i = vectorToInt64Map(Qkmers_i_sorted, i);
    QkmerMap_n[i] = QkmerMap_i;
  }
  //combine maps
  QkmerMap = combineInt64Maps(QkmerMap_n);
  ...
  std::vector<std::map<std::int64_t, int>> TkmerMap_m(m);
  TkmerMap_m.resize(m);
  for (int j = 0; j < m; ++j) {
    std::map<std::int64_t, int> TkmerMap_j;
    std::vector<std::int64_t> Tkmers_j_sorted = Tkmers_sorted[j];
    TkmerMap_j = vectorToInt64Map(Tkmers_j_sorted, j);
    TkmerMap_m[j] = TkmerMap_j;
  }
  //combine maps
  TkmerMap = combineInt64Maps(TkmerMap_m);
  ...
  std::map<std::int64_t, std::vector<int>> subsetQkmerMap;
  subsetQkmerMap = getSubsetInt64Map(QkmerMap, sparse_n);
  std::map<std::int64_t, std::vector<int>> subsetTkmerMap;
  subsetTkmerMap = getSubsetInt64Map(TkmerMap, sparse_n);
  std::vector<std::vector<int>> sparse_pairs(n);
  sparse_pairs = findPairsInt64_sparse(subsetQkmerMap, subsetTkmerMap, n);
  ...
  if (QkmerMap.size() < TkmerMap.size()) {
    if (use_sparse) {
      Q_T_pairs = findPairsInt64_sparse(QkmerMap, TkmerMap, n);
    } else {
      for (int i = 0; i < n; ++i) {
        Q_T_pairs[i] = findPairsInt64(QkmerMap_n[i], TkmerMap);
      }
    }
  } else {
    std::vector<std::vector<int>> T_Q_pairs(m);
    if (use_sparse) {
      T_Q_pairs = findPairsInt64_sparse(TkmerMap, QkmerMap, m);
    } else {
      for (int j = 0; j < m; ++j) {
        T_Q_pairs[j] = findPairsInt64(TkmerMap_m[j], QkmerMap);
      }
    }
  ...
  for (int ni_mj = 0; ni_mj < candidatePairs_ni_mj_size; ++ni_mj) {
    ...
    Qni_Tmj_distances[ni_mj]=getJaccardByIntegerVectorInt64(
      Qkmers_sorted[Qni],
      Tkmers_sorted[Tmj],
